...
const ImageFrame: React.FC<ImageFrameProps> = ({ isOpen }) => {
  ...
  if (!drawingContext || !toolContext) {
    throw new Error("DrawingContext and ToolContext must be used within seus respectivos providers");
  }
  ...
  const handlePointerDown = () => {
    ...
    drawingDispatch({ type: 'POINTER_DOWN', point , selectedTool: toolState.selectedTool });
  };
      ...
      >
        <Layer>
          <Rect
            ...
            fill="lightgray"
          />
          {drawingState.items.map((item: GeometryItem, index: number) => (
            <Geometry item={item} index={index} key={index}/>
          ))}
          {drawingState.previewItem && <GeometryPreview previewItem={drawingState.previewItem} />}
        </Layer>
      </Stage>
    </div>
  );
